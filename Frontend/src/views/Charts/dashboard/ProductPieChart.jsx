...
const ProductPieChart = (props) => {
    ...
    const getProductData = (dateRange = [], platform) => {
        // if (flag %2 === 0)
        services.getProductData({
            ...
            for (let i = 0; i < response_raw.length; i++) {
                data_response.push({key: response_raw[i].product_name, value: response_raw[i].count, color: COLORS[i]})
            }
    ...
    const callApi = () =>{
        services.callApi().then(res => console.log(res)).catch(err => console.log(err));
    }

    useEffect(() => {
        // callApi();
        // getApiData(globalDateRange, globalPlatform);
        getProductData(globalDateRange, globalPlatform);
